...
// // GET /api/availability/micro-slots?date=2026-03-19&slotTime=09:00 - 12:00
...
//     // Find which micro slots are already booked
...
//     // ✅ Use citizenId for duplicate check (most reliable); fallback to mobileNumber
//     const duplicateFilter = citizenId
//       ? { citizenId, preferredDate, status: { $nin: ["rejected", "expired"] } }
//       : { mobileNumber, preferredDate, status: { $nin: ["rejected", "expired"] } };
 
//     const existingBookings = await CitizenAppointment.find(duplicateFilter);
//     if (existingBookings.length > 0) {
//       return res.status(409).json({
//         success: false,
//         message: "त्याच date साठी आधीच booking आहे ❌",
...
//     // ✅ FIXED: Block only if citizen already has an active booking on this date
//     // for a DIFFERENT microSlot — allows booking any other available slot freely
//     const effectiveCitizenId = citizenId && citizenId.toString().trim() !== "" ? citizenId : null;

//     const duplicateFilter = effectiveCitizenId
//       ? { citizenId: effectiveCitizenId, preferredDate, microSlot: { $ne: microSlot }, status: { $nin: ["rejected", "expired"] } }
//       : { mobileNumber, preferredDate, microSlot: { $ne: microSlot }, status: { $nin: ["rejected", "expired"] } };
...
exports.bookAppointment = async (req, res) => {
  try {
    ...
    // ✅ Only block if this exact 15-min micro slot is already taken by anyone
    const microSlotTaken = await CitizenAppointment.findOne({
      preferredDate,
      microStart,
      microEnd,
    ...
    // ✅ Check if parent slot is fully booked (all 15-min slots taken)
    const totalMicroSlots = allMicroSlots.length;
    const bookedCount = await CitizenAppointment.countDocuments({
      preferredDate,
      slotTime,
